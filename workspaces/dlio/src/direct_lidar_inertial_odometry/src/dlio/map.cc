...
dlio::MapNode::MapNode(): Node("dlio_map_node") {
  ...
  this->map_pub = this->create_publisher<sensor_msgs::msg::PointCloud2>("odom", 100);
  ...
  this->last_map_update = rclcpp::Time(0, 0, this->get_clock()->get_clock_type());
...
void dlio::MapNode::getParams() {
  ...
  this->declare_parameter<std::string>("odom/odom_frame", "odom_dlio");
  this->declare_parameter<double>("map/sparse/leafSize", 0.5);
  this->declare_parameter<double>("map/publish_freq", 1.0);

  this->get_parameter("odom/odom_frame", this->odom_frame);
  this->get_parameter("map/sparse/leafSize", this->leaf_size_);
  this->get_parameter("map/publish_freq", this->map_publish_freq_);
}
...
void dlio::MapNode::callbackKeyframe(const sensor_msgs::msg::PointCloud2::ConstSharedPtr& keyframe) {
  ...
  *this->dlio_map += *keyframe_pcl;
  ...
  if (this->dlio_map->points.size() == this->dlio_map->width * this->dlio_map->height) {
    rclcpp::Time keyframe_time(keyframe->header.stamp);
    double time_diff = keyframe_time.seconds() - this->last_map_update.seconds();
    if (time_diff >= 1.0 / this->map_publish_freq_) {
      sensor_msgs::msg::PointCloud2 map_ros;
      pcl::toROSMsg(*this->dlio_map, map_ros);
      map_ros.header.stamp = keyframe_time;
      map_ros.header.frame_id = this->odom_frame;
      this->map_pub->publish(map_ros);
      this->last_map_update = keyframe_time;
    }
  } else {
     RCLCPP_WARN(this->get_logger(), "Map inconsistent structure!");
  }
}
