...
void dlio::OdomNode::getParams() {
  ...
  dlio::declare_param(this, "frames/baselink", this->baselink_frame, "sensor");
  dlio::declare_param(this, "frames/lidar", this->lidar_frame, "sensor");
...
void dlio::OdomNode::publishToROS(pcl::PointCloud<PointType>::ConstPtr published_cloud, Eigen::Matrix4f T_cloud) {
  ...
  // transform: baselink to imu (only if different frames)
  if (this->baselink_frame != this->imu_frame) {
    transformStamped.header.stamp = this->imu_stamp;
  ...
  // transform: baselink to lidar (only if different frames)
  if (this->baselink_frame != this->lidar_frame) {
    transformStamped.header.stamp = this->imu_stamp;
    transformStamped.header.frame_id = this->baselink_frame;
  ...
  if (this->deskew_) {
    ...
    if (!this->first_valid_scan) {
      ...
      if (this->imu_buffer.empty()) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "IMU buffer is empty, waiting for IMU data...");
        return;
      }
      
      // Check if scan is newer than the NEWEST IMU data (front, since push_front is used)
      // Add 50ms tolerance for simulation timing jitter
      if (this->scan_stamp > this->imu_buffer.front().stamp + 0.05) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 1000, 
          "Scan stamp (%.3f) > IMU front stamp (%.3f) - waiting for IMU. Buffer size: %zu",
          this->scan_stamp, this->imu_buffer.front().stamp, this->imu_buffer.size());
        return;
      }

      this->first_valid_scan = true;
      this->T_prior = this->T; // assume no motion for the first scan
      std::cout << "[DLIO] First valid scan accepted (no-deskew mode)" << std::endl;
